...
class AttractionsSchema(BaseModel):
    attractions: list[str] = Field(description="List of attractions found from search")


class RankedAttractionsSchema(BaseModel):
    ranked: list[str] = Field(description="Attractions ranked by interests and ratings")


class SelectedAttractionsSchema(BaseModel):
